...
function InterviewCard({ item, color, query }: { item: InterviewItem; color: InterviewSection["color"]; query: string }) {
  ...
  const termClass =
    color === "yellow"
      ? "text-yellow-300 font-bold text-[12px]"
      : color === "red"
        ? "text-red-300 font-bold text-[12px]"
  ...
  return (
    <div className="border border-zinc-700 bg-[#141414] rounded mb-2 break-inside-avoid">
      <button
        ...
        onClick={() => setOpen((v) => !v)}
      >
        <span className="text-zinc-600 text-[10px] mt-[2px] shrink-0 select-none">
          {isOpen ? "▾" : "▸"}
        </span>
        <div className="flex-1 min-w-0">
          <span className={termClass}>{highlight(item.term, query)}</span>
          <span className="text-zinc-400 text-[11px] ml-2 leading-snug">
        ...
        <ul className="px-3 pb-2 space-y-[3px] border-t border-zinc-800 pt-1.5">
          {item.detail.map((d, i) => (
            <li key={i} className="flex gap-1.5 items-baseline">
              ...
              <span className="text-zinc-200 text-[11px] leading-snug font-mono">
                {highlight(d, query)}
              </span>
            </li>
          ))}
